...
`
	...
	t.Run("Valid tree passes validation", func(t *testing.T) {
		...
		key0, _ := schema.OrderToKey(0)
		key1, _ := schema.OrderToKey(1)
		key2, _ := schema.OrderToKey(2)
		...
		valid, err := multiprover.Validate(rdfSchema, tree)
	...
	t.Run("Tree with unexpected index fails validation", func(t *testing.T) {
		...
		unexpectedKey, _ := schema.OrderToKey(10) // order 10 not in schema
...
`
		...
		valid, err := multiprover.Validate(boolSchema, tree)
		require.NoError(t, err)
		assert.True(t, valid)
		...
		valid, err = multiprover.Validate(boolSchema, tree2)
		require.Error(t, err)
		assert.False(t, valid)
...
`
		...
		valid, err := multiprover.Validate(intSchema, tree)
		require.NoError(t, err)
		assert.True(t, valid)
		...
		valid, err = multiprover.Validate(intSchema, tree2)
		require.Error(t, err)
		assert.False(t, valid)
...
`
		...
		valid, err := multiprover.Validate(extrinsicSchema, tree)
		require.NoError(t, err)
		assert.True(t, valid)
		...
		valid, err = multiprover.Validate(extrinsicSchema, tree2)
		require.Error(t, err)
		assert.False(t, valid)
	...
	t.Run("Skip type verification for encrypted data", func(t *testing.T) {
		...
		valid, err := multiprover.ValidateWithOptions(rdfSchema, tree, true)
		require.NoError(t, err)
		assert.True(t, valid)
		
		// Without skipTypeVerification, this should fail
		valid, err = multiprover.ValidateWithOptions(rdfSchema, tree, false)
		require.Error(t, err)
		assert.False(t, valid)
