	...
	for _, listenAddr := range listenAddrs {
		...
		case manet.IsIPUnspecified(natAddr):
			...
			for _, addr := range obsAddrs {
				...
				if obsIP != nil {
					if pubadd, err := manet.IsPublicAddr(obsIP.Multiaddr()); err == nil && pubadd {
		...
		default:
			...
			if pubadd, err := manet.IsPublicAddr(natAddr); !pubadd && err == nil { // nat reported non public addr(maybe CGNAT?)
