...
func TestAppendNATAddrs(t *testing.T) {
	if1, if2 := tStringCast("/ip4/192.168.0.100"), tStringCast("/ip4/1.1.1.1")
	ifaceAddrs := []ma.Multiaddr{if1, if2}
	tcpListenAddr, udpListenAddr := tStringCast("/ip4/0.0.0.0/tcp/1"), tStringCast("/ip4/0.0.0.0/udp/2/quic-v1")
	cases := []struct {
		Name        string
		Listen      ma.Multiaddr
		Nat         ma.Multiaddr
	...
	}{
		{
			...
			Listen: tStringCast("/ip4/0.0.0.0/udp/1/quic-v1"),
			Nat:    tStringCast("/ip4/1.1.1.1/udp/10/quic-v1"),
			ObsAddrFunc: func(m ma.Multiaddr) []ma.Multiaddr {
				return []ma.Multiaddr{tStringCast("/ip4/2.2.2.2/udp/100/quic-v1")}
			},
			Expected: []ma.Multiaddr{tStringCast("/ip4/1.1.1.1/udp/10/quic-v1")},
		},
		{
			Name: "nat map failure",
			// nat mapping fails, obs addresses added
			Listen: tStringCast("/ip4/0.0.0.0/tcp/1"),
			Nat:    nil,
			ObsAddrFunc: func(a ma.Multiaddr) []ma.Multiaddr {
				...
				switch {
				case ip.Equal(if1):
					return []ma.Multiaddr{tStringCast("/ip4/2.2.2.2/tcp/100")}
				case ip.Equal(if2):
					return []ma.Multiaddr{tStringCast("/ip4/3.3.3.3/tcp/100")}
				default:
					return []ma.Multiaddr{}
				}
			},
			Expected: []ma.Multiaddr{tStringCast("/ip4/2.2.2.2/tcp/100"), tStringCast("/ip4/3.3.3.3/tcp/100")},
		},
		{
			Name: "if addrs ignored if not listening on unspecified",
			// nat mapping fails, obs addresses added
			Listen: tStringCast("/ip4/192.168.1.1/tcp/1"),
			Nat:    nil,
			ObsAddrFunc: func(a ma.Multiaddr) []ma.Multiaddr {
				ipC, _ := ma.SplitFirst(a)
				ip := ipC.Multiaddr()
				...
				case ip.Equal(if2):
					return []ma.Multiaddr{tStringCast("/ip4/3.3.3.3/tcp/100")}
				case ip.Equal(tStringCast("/ip4/192.168.1.1")):
					return []ma.Multiaddr{tStringCast("/ip4/4.4.4.4/tcp/100")}
				default:
					return []ma.Multiaddr{}
				}
			},
			Expected: []ma.Multiaddr{tStringCast("/ip4/4.4.4.4/tcp/100")},
		},
		{
			Name: "nat map success but CGNAT",
			// nat addr added, obs address added with nat provided port
			Listen: tcpListenAddr,
			Nat:    tStringCast("/ip4/100.100.1.1/tcp/100"),
			ObsAddrFunc: func(a ma.Multiaddr) []ma.Multiaddr {
				ipC, _ := ma.SplitFirst(a)
				ip := ipC.Multiaddr()
				if ip.Equal(if1) {
					return []ma.Multiaddr{tStringCast("/ip4/2.2.2.2/tcp/20")}
				}
				return []ma.Multiaddr{tStringCast("/ip4/3.3.3.3/tcp/30")}
			},
			Expected: []ma.Multiaddr{
				tStringCast("/ip4/100.100.1.1/tcp/100"),
				tStringCast("/ip4/2.2.2.2/tcp/20"),
				tStringCast("/ip4/3.3.3.3/tcp/30"),
			},
		},
		{
			...
			ObsAddrFunc: func(a ma.Multiaddr) []ma.Multiaddr {
				if manet.IsIPUnspecified(a) {
					return []ma.Multiaddr{tStringCast("/ip4/3.3.3.3/udp/20/quic-v1")}
				}
				return []ma.Multiaddr{tStringCast("/ip4/2.2.2.2/udp/20/quic-v1")}
			},
			Expected: []ma.Multiaddr{
				tStringCast("/ip4/2.2.2.2/udp/20/quic-v1"),
				tStringCast("/ip4/3.3.3.3/udp/20/quic-v1"),
			},
		},
	}
...
func TestAddrsManager(t *testing.T) {
	lhquic := tStringCast("/ip4/127.0.0.1/udp/1/quic-v1")
	lhtcp := tStringCast("/ip4/127.0.0.1/tcp/1")

	publicQUIC := tStringCast("/ip4/1.2.3.4/udp/1/quic-v1")
	publicTCP := tStringCast("/ip4/1.2.3.4/tcp/1")
	...
	t.Run("nat returns unspecified addr", func(t *testing.T) {
		quicPort1 := tStringCast("/ip4/3.3.3.3/udp/1/quic-v1")
		quicPort2 := tStringCast("/ip4/3.3.3.3/udp/2/quic-v1")
		// port from nat, IP from observed addr
		am := newAddrsManagerTestCase(t, addrsManagerArgs{
			NATManager: &mockNatManager{
				GetMappingFunc: func(addr ma.Multiaddr) ma.Multiaddr {
					if addr.Equal(lhquic) {
						return tStringCast("/ip4/0.0.0.0/udp/2/quic-v1")
					}
					return nil
				},
			},
	...
	t.Run("public addrs removed when private", func(t *testing.T) {
		...
		relayAddr := tStringCast("/ip4/1.2.3.4/udp/1/quic-v1/p2p/QmdXGaeGiVA745XorV1jr11RHxB9z4fqykm6xCUPX1aTJo/p2p-circuit")
	...
	t.Run("addrs factory gets relay addrs", func(t *testing.T) {
		relayAddr := tStringCast("/ip4/1.2.3.4/udp/1/quic-v1/p2p/QmdXGaeGiVA745XorV1jr11RHxB9z4fqykm6xCUPX1aTJo/p2p-circuit")
		publicQUIC2 := tStringCast("/ip4/1.2.3.4/udp/2/quic-v1")
		am := newAddrsManagerTestCase(t, addrsManagerArgs{
